...
2. ![](../assets/images/umami-serverless_1.webp)    ，通过`Sync fork`同步仓库
...
2. 导入仓库  ![](../assets/images/umami-serverless_2.webp)    ![](../assets/images/umami-serverless_3.webp)

3. 构建Umami![](../assets/images/umami-serverless_4.webp)，正常来说会报错
...
5. `Settings` - `Environment Variables` 中进行设置环境变量，![](../assets/images/umami-serverless_5.webp)在`Key`中填入`DATABASE_URL`，`Value`中填入类似`postgresql://neondb_owner:xxx@ep-round-base-a49lcqo5-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require`（在主页的`Storage`的数据库内可以找到）